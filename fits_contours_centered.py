...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug  6 16:19:52 2019
...
image_gr_q = image_gr.copy()
image_ri_q = image_ri.copy()
image_iz_q = image_iz.copy()
image_gz_q = image_gz.copy()

from scipy.ndimage.filters import gaussian_filter

image_gr = gaussian_filter(image_gr, sigma=3)
fits.writeto('gr_model.fits',image_gr,overwrite=True)
image_ri = gaussian_filter(image_ri, sigma=3)
fits.writeto('ri_model.fits',image_ri,overwrite=True)
image_iz = gaussian_filter(image_iz, sigma=3)
fits.writeto('iz_model.fits',image_iz,overwrite=True)
image_gz = gaussian_filter(image_gz, sigma=3)
fits.writeto('gz_model.fits',image_gz,overwrite=True)
...
#image_iz[:,:] /= np.repeat(np.array([samples.shape[0]]), np.array(image_iz.shape))
#image_gz[:,:] /= np.repeat(np.array([samples.shape[0]]), np.array(image_gz.shape))
image_gr_p = image_gr.copy()
image_ri_p = image_ri.copy()
image_iz_p = image_iz.copy()
image_gz_p = image_gz.copy()

image_gr = gaussian_filter(image_gr, sigma=3)
fits.writeto('gr_orig.fits',image_gr,overwrite=True)
image_ri = gaussian_filter(image_ri, sigma=3)
fits.writeto('ri_orig.fits',image_ri,overwrite=True)
image_iz = gaussian_filter(image_iz, sigma=3)
fits.writeto('iz_orig.fits',image_iz,overwrite=True)
image_gz = gaussian_filter(image_gz, sigma=3)
fits.writeto('gz_orig.fits',image_gz,overwrite=True)
...
# Compute KL-Divergence using two scaled images

#image_gr_q = np.where(image_gr_q < np.amin(image_gr_p[np.where(image_gr_p != 0)]), 0, image_gr_q)
image_gr_div = np.where(image_gr_q != 0, image_gr_p/image_gr_q, 1)
image_gr_diff = np.where(image_gr_p != 0, image_gr_p * np.log(image_gr_div), 0)
print(np.sum(image_gr_diff))

image_ri_div = np.where(image_ri_q != 0, image_ri_p/image_ri_q, 1)
image_ri_diff = np.where(image_ri_p != 0, image_ri_p * np.log(image_ri_div), 0)
print(np.sum(image_ri_diff))

image_iz_div = np.where(image_iz_q != 0, image_iz_p/image_iz_q, 1)
image_iz_diff = np.where(image_iz_p != 0, image_iz_p * np.log(image_iz_div), 0)
print(np.sum(image_iz_diff))

image_gz_div = np.where(image_gz_q != 0, image_gz_p/image_gz_q, 1)
image_gz_diff = np.where(image_gz_p != 0, image_gz_p * np.log(image_gz_div), 0)
print(np.sum(image_gz_diff))

with open('fits_kl_values.txt', 'w') as filehandle:
    filehandle.write('kl value for gr: ' + str(np.sum(image_gr_diff)) + '\n')
    filehandle.write('kl value for ri: ' + str(np.sum(image_ri_diff)) + '\n')
    filehandle.write('kl value for iz: ' + str(np.sum(image_iz_diff)) + '\n')
    filehandle.write('kl value for gz: ' + str(np.sum(image_gz_diff)) + '\n')
